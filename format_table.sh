...
{
    echo "
    用途：将数据格式化为表格输出，支持自定义分隔符、标题和行数据
    特征码：
        ${GAN_WHAT_FUCK:-'未命名'}
    权限要求：
        ${NEED_PRIVILEGES:-'未指定'}
    ...
    用法：
        \$0  --help|-h                                                         #-- 帮助
        \$0  [{--delimeter|-d <分隔符>}]  [{--title|-t <字段名...>}] \\
             [{--row|-r <值...>}]  [{--file|-f <文件>}]                      #-- 默认分隔符为【,】
    参数规范：
        |          : val1|val2|<valn>  : 多选一
        <>         : <val>             : 需替换的具体值（用户必须提供）
    ...
    参数说明：
        --help|-h                    # 帮助
        --delimeter|-d <分隔符>      # 表格分隔符，默认为【,】
        --title|-t <字段名...>       # 标题，如：'aa, bb, cc'
        --row|-r <值...>             # 行数据，可多次指定，如：'aa, bb, cc'
        --file|-f <文件>             # 从文件读取数据
    示例：
        \$0          -f 文件1          #-- 默认分隔符，标题与内容皆取自【文件1】
        \$0  -d '|'  -f 文件1          #-- 分隔符为【|】，标题与内容皆取自【文件1】
        #
        \$0  -d '|'  -t 'A|B|C'  -r 'aa|bb|cc'  -r 'vv| |xx'    #-- 定义标题，内容取自参数
        \$0  -d '|'  -t 'A|B|C'  -f 文件1                       #-- 定义标题，内容取自文件
"
}
